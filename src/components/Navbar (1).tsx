...
export function Navbar() {
  ...
  const casesPath = isAdmin ? '/admin/cases' : '/volunteer/cases';
  const alertsPath = isAdmin ? '/admin/alerts' : '/volunteer#alerts';
    ...
    <header className="sticky top-0 z-10 flex h-16 shrink-0 items-center justify-between border-b border-slate-200/80 bg-white/90 px-6 backdrop-blur transition-colors dark:border-gray-800 dark:bg-[#0a0a0a]/90">
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          ...
          title="View alerts"
          className="relative rounded-full p-2 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-gray-800 dark:hover:text-gray-200"
        >
          <Bell size={20} />
          <span className="absolute right-1.5 top-1.5 h-2 w-2 rounded-full border-2 border-white bg-red-500 dark:border-gray-900"></span>
        </button>
        ...
        <div className="flex items-center gap-3 pl-2">
          ...
          <button
            ...
            onClick={logout}
            title="Sign out"
            className="rounded-full p-2 text-slate-400 transition-colors hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-500/10 dark:hover:text-red-400"
          >
            <LogOut size={18} />
          </button>
        </div>
